  ...
  describe('/api/users', function () {
    describe('POST', function () {
      ...
      it('Should create a new user', function () {
        ...
        return chai.request(app).post('/api/users').send(testUser)
          ...
          .then(user => {
            ...
            expect(String(user._id)).to.equal(res.body.id);
      ...
      it('Should reject users with non-string username', function () {
        const testUser = { username: 123, password, fullname };
        return chai.request(app).post('/api/users').send(testUser)
          .catch(err => err.response)
          .then(res => {
            expect(res).to.have.status(422);
      ...
      it('Should reject users with non-trimmed username', function () {
        const testUser = { username: ' kevin   ', password, fullname };
        return chai.request(app).post('/api/users').send(testUser)
          .catch(err => err.response)
          .then(res => {
            expect(res).to.have.status(422);
      ...
      it('Should reject users with non-trimmed password', function () {
        const testUser = { username, password: ' kevin   ', fullname };
        return chai.request(app).post('/api/users').send(testUser)
          .catch(err => err.response)
          .then(res => {
            expect(res).to.have.status(422);
      ...
      it('Should reject users with password greater than 72 characters', function () {
        const password = new Array(73).fill('1').join('');
        const testUser = { username, password, fullname };
        return chai.request(app).post('/api/users').send(testUser)
          .catch(err => err.response)
          .then(res => {
      ...
      it('Should reject users with duplicate username', function () {
        const testUser = { username: 'Bill', password, fullname };
      ...
      it('Should trim fullname', function () {
        const testUser = { username, password, fullname: '  kevin murphy  ' };
        return chai.request(app).post('/api/users').send(testUser)
          .catch(err => err.response)
          .then(res => {
            expect(res).to.have.status(201);
            expect(res.body).to.be.an('object');
            expect(res.body.fullname).to.equal('kevin murphy');
          });
      });
